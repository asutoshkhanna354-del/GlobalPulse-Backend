...
export async function refreshNiftyComprehensive(): Promise<{ direction: string; confidence: number }> {
  ...
  await db.insert(niftyAnalysisTable).values({
    ...
    supportLevels: finalData.stoploss ? [String(finalData.stoploss)] : [],
    resistanceLevels: finalData.target1 ? [String(finalData.target1)] : [],
    keyFactors: finalData.reason ? [finalData.reason] : [],
    demandZones: [],
    supplyZones: [],
    candlePattern: "N/A",

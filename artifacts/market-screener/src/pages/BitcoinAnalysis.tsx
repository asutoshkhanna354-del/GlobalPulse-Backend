      ...
      <div className="absolute inset-0 bg-background/80 backdrop-blur-sm z-10 flex items-center justify-center rounded-xl">
        <div className="text-center p-8 max-w-md">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Bitcoin AI Analysis with Long/Short recommendations is available exclusively for Pro users.
          </p>
          <button
            onClick={() => setShowActivation(true)}
            className="px-6 py-2.5 bg-orange-500 text-white font-semibold rounded-lg hover:bg-orange-600 transition-colors flex items-center gap-2 mx-auto"
          >
...
export function BitcoinAnalysis() {
  const { isPremium } = usePremium();

  const { data, isLoading, refetch, isFetching } = useQuery({
    queryKey: ["bitcoin-analysis"],
    queryFn: async () => {
  ...
  const { data: history } = useQuery({
    queryKey: ["bitcoin-analysis-history"],
    queryFn: async () => {
      const res = await fetch(`${API}/bitcoin-analysis/history?type=candle_4h&limit=10`);
      if (!res.ok) throw new Error("Failed");
      return res.json();
    },
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-lg sm:text-xl font-semibold text-foreground">Bitcoin AI Analysis</h1>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            AI-powered comprehensive outlook + periodic 4-hour demand-supply candle analysis for Bitcoin trading
          </p>
        </div>
        ...
        <ProGate>
          <div className="space-y-6">
              ...
              <div className="bg-card border border-border rounded-xl p-5 space-y-5">
                ...
                {comp.btcPrice && (
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="text-2xl font-bold font-mono text-foreground">
                      ${comp.btcPrice.toLocaleString("en-US", { maximumFractionDigits: 0 })}
                    </div>
                    <div className={`flex items-center gap-1 text-lg font-semibold ${(comp.btcChange ?? 0) >= 0 ? "text-green-600" : "text-red-600"}`}>
                      {(comp.btcChange ?? 0) >= 0 ? <ArrowUpRight className="w-5 h-5" /> : <ArrowDownRight className="w-5 h-5" />}
                      {(comp.btcChange ?? 0) > 0 ? "+" : ""}{(comp.btcChange ?? 0).toFixed(2)}%
                    </div>
                  </div>
                )}
                ...
                {comp.outlook && (
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-orange-800 leading-relaxed whitespace-pre-line">{comp.outlook}</p>
                  </div>
                )}
                ...
                {comp.tradeRecommendation && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                    <div className="flex items-center gap-1.5 mb-2 text-[11px] font-bold uppercase tracking-widest text-amber-700">
                      <Zap className="w-3.5 h-3.5" /> Trade Recommendation
                    </div>
                    <p className="text-sm text-amber-800 font-semibold">{comp.tradeRecommendation}</p>
                    <div className="flex gap-4 mt-2 text-xs text-amber-700">
                      {comp.targetPrice && <span>Target: ${comp.targetPrice.toLocaleString("en-US")}</span>}
                      {comp.stopLoss && <span>Stop Loss: ${comp.stopLoss.toLocaleString("en-US")}</span>}
                    </div>
                  </div>
                )}
              ...
              <div className="bg-card border border-border rounded-xl p-5 space-y-5">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-base font-bold text-foreground">4-Hour Candle Analysis</h2>
                ...
                <div>
                  <div className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-1">4h Candle Summary</div>
                ...
                {candle.outlook && (
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                    <div className="flex items-center gap-1.5 mb-2 text-[11px] font-bold uppercase tracking-widest text-orange-700">
                      <AlertTriangle className="w-3.5 h-3.5" /> Next 4-8 Hour Outlook
                    </div>
                ...
                {candle.tradeRecommendation && (
                  <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-emerald-800 font-semibold">{candle.tradeRecommendation}</p>
                    <div className="flex gap-4 mt-2 text-xs text-emerald-700">
                      {candle.targetPrice && <span>Target: ${candle.targetPrice.toLocaleString("en-US")}</span>}
                      {candle.stopLoss && <span>SL: ${candle.stopLoss.toLocaleString("en-US")}</span>}
                    </div>
                  </div>
                )}
            ...
            {isPremium && history?.length > 0 && (
              <div className="bg-card border border-border rounded-xl p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-bold text-foreground">Recent 4h Analysis History</h3>
                </div>
                    ...
                    <tbody>
                      {history.map((h: any) => (
                        <tr key={h.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-3 py-2 font-mono text-xs">${h.btcPrice?.toLocaleString("en-US") ?? "—"}</td>
                          <td className="px-3 py-2 text-xs text-muted-foreground truncate max-w-[200px]">{h.tradeRecommendation ?? "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="bg-muted/50 border border-border rounded-lg p-4 text-[11px] text-muted-foreground space-y-1">
              ...
              <p>This analysis is AI-generated for educational purposes only. It does not constitute financial advice. Always do your own research before trading. Bitcoin is highly volatile and carries significant risk. Past performance does not guarantee future results.</p>
              <p>Analysis refreshes: Comprehensive every 1 hour, 4-hour candle analysis every 4 hours. All times in IST.</p>
            </div>
          </div>
        </ProGate>
      )}
